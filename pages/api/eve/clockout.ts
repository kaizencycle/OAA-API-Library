...
async function sealToLedger(sha256: string, meta: { companion: string; cycle: string; }) {
  ...
  const body = { title:`Eve Clock-out ${meta.cycle}`, slug:`clockout-${meta.cycle}`, companion:meta.companion, sha256 };
...
/**
 * POST /api/eve/clockout
 * Headers: x-hmac-sha256 signature of raw JSON body
 * Body: { "cycle":"C-108", "companion":"eve", "wins":["..."], "blocks":["..."], "tomorrowIntent":["..."], "meta":{"tz":"ET"} }
 */
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  try{
    ...
    const { cycle, companion="eve", wins=[], blocks=[], tomorrowIntent=[], meta={} } = req.body||{};
    if(!cycle) return res.status(400).json({ok:false,error:"missing_cycle"});
    const now=new Date().toISOString();
    const digest=`# Eve Clock-out — ${cycle}\nat: ${now}\nwins:\n${wins.map((w:string)=>`- ${w}`).join("\n")||"- (none)"}\nblocks:\n${blocks.map((b:string)=>`- ${b}`).join("\n")||"- (none)"}\ntomorrow:\n${tomorrowIntent.map((t:string)=>`- ${t}`).join("\n")||"- (none)"}`;
    const sha=sha256hex(digest);
    const m=readMemory();
    m.notes.unshift({type:"eve.clockout",cycle,companion,sha256:sha,ts:Date.now(),digest});
    writeMemory(m);
    const seal=await sealToLedger(sha,{companion,cycle});
    
    // Auto-open next cycle stub
    try {
      const next = String(cycle).replace(/C-(\d+)/, (_m,g)=>`C-${Number(g)+1}`);
      const m2 = readMemory();
      m2.notes.unshift({
        type: "eve.stub",
        cycle: next,
        companion,
        ts: Date.now(),
        note: `Stub opened for ${next}`,
        tomorrowIntent: tomorrowIntent
      });
      writeMemory(m2);
    } catch {}
    ...
    return res.status(500).json({ok:false,error:e?.message||"clockout_error"});
  }
}